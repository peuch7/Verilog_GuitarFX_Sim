...
`delay` and `chorus` are implemented; the other four effects are passthrough
stubs with complete float reference models that specify what they should do.

`chorus` scores ~85 dB against its reference at 1 kHz. The residual is the
1024-point LFO sine table's chord-versus-arc interpolation error, which is a
deliberate BRAM tradeoff, not a bug. Its fixed-point constants are generated by
`scripts/gen_sine_table.py` rather than typed in: a hand-computed phase constant
that was 60 ppm off once passed every short test while pulling the LFO a full
sample out of step over a few seconds. Regenerate rather than editing
`rtl/effects/chorus/chorus_sine_table.svh`, which is generated.
